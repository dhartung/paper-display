...
void write_text(String string, int offset) {
  if (print_on_display) {
    cursor_x += offset;
    writeln((GFXfont *)&OpenSans16, (char *)string.c_str(), &cursor_x,
            &cursor_y, NULL);
    cursor_x = 200;
    cursor_y += 35;
    delay(100);  // Make sure text is seen and not directly overwritten.
  }
...
void write_error(String string, bool clearArea) {
  ...
  uint8_t *framebuffer =
      (uint8_t *)ps_calloc(sizeof(uint8_t), area.height * area.width / 2);
...
void write_error(String string) { write_error(string, true); }
