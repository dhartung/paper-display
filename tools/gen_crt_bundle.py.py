#!/usr/bin/env python
#
# Original script taken from: https://github.com/espressif/esp-idf/blob/bab38307974b19cbeb933c6eafdeec461724124b/components/mbedtls/esp_crt_bundle/gen_crt_bundle.py
# Modified to simplify filtering and importing certificates from system.
#
...
ca_bundle_bin_file = os.path.join(current_location, '..', 'data', 'cert', 'x509_crt_bundle.bin')
...
class CertificateBundle:
    ...
    def add_from_system(self):
        self.add_from_pem(certifi.contents())

    def add_from_path(self, crts_path: str):
    ...
    def add_from_pem(self, crt_str: str):
    ...
    def filter(self, regex: re.Pattern):
        self.certificates = [x for x in self.certificates if regex.search(x.issuer.rfc4514_string())]
...
def main():
    ...
    if args.input:
        ...
        bundle.add_from_system()

    if args.filter:
        bundle.filter(re.compile(args.filter))
